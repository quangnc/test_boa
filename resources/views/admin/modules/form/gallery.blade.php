        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ $data['action'] }}" method="POST" id="{{ $data['form'] }}">
                    ...
                    <div class="row" style="margin-bottom: 20px">
                        ...
                        <div class="col-10">
                            <input type="text" name="module[name]"
                                   value="{{ isset($data['name']) ? $data['name'] : '' }}" placeholder="Module Name"
                    ...
                    <div class="row" style="margin-bottom: 20px">
                        <label class="col-2 text-right" for="input-name">Type</label>
                        <div class="col-10">
                            <select class="form-control" name="module[type]">
                                @foreach($data['type'] as $key => $option)
                                    <option value="{{ $key }}" {{ $data['type'] === $key ? 'selected' : "" }}>{{ $option }}</option>
                                @endforeach 
                            </select>
                        </div>
                    </div>

                    <gallery input-name="module[images]"
                             :value="{{ isset($data['images']) ? json_encode($data['images']) : '[]'}}"/>
