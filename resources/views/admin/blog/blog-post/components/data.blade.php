...
<div class="card card-primary">
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="blog_post[blog_category_id]">
                @isset($blog_post_categories)
                @foreach($blog_post_categories as $blog_post_category)
                    <option 
                        value="{{ $blog_post_category->blog_category_id }}" 
                        {{ isset($blog_post) && $blog_post->blog_category_id == $blog_post_category->blog_category_id ? "selected" : ""  }}
                    >
        ...
        <div class="form-group">
            <label for="txtBlogTag">Blog Tag</label>
            <input 
                value="{{ isset($blog_tag->name) ? $blog_tag->name : old('blog_tag.name') }}" 
                type="text" 
                class="form-control" 
                id="txtBlogTag" 
                ...
                name="blog_tag[name]" 
            >
            @if($errors->has('blog_tag.name') && !old("blog_tag.name"))
                <p style="color:red">{{ $errors->first('blog_tag.name') }}</p>
            @endif
        </div>
    </div>
</div>