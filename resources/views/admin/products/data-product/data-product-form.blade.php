        ...
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ $data['action'] }}" method="POST" id="{{ $data['form'] }}">
                    {{ csrf_field() }}
                    @if ($data['action'] != route( 'data-product.index' )) 
                        <input name="_method" type="hidden" value="PATCH">
                    @endif
                    
                    <div class="tab-content" style="padding-top: 20px">
                        <div class="tab-pane active" id="general">
                            @include('admin.products.data-product.components.general')
                        </div>
                        <div class="tab-pane" id="data">
                            @include('admin.products.data-product.components.data')
                        </div>
                        {{-- <div class="tab-pane" id="image">
                            @include('admin.products.data-product.components.image')
                        </div> --}}
                    </div>
                </form>
            </div>
