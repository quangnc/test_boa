<ul class="menu">
<li class="first leaf active-trail"><a href="{{ route('laboratories-introduction') }}" title="" class="active-trail active">Giới thiệu</a></li>
<li class="leaf"><a href="{{ route('laboratories-search') }}" title="">Tìm kiếm</a></li>
<li class="leaf"><a href="{{ route('laboratories-application') }}" title="">Biểu mẫu</a></li>
<li class="leaf"><a href="{{ route('laboratories-document') }}" title="">Tài liệu</a></li>
<li class="last leaf"><a href="{{ route('laboratories-index') }}" title="">Gia hạn, đình chỉ, hủy bỏ</a></li>
</ul>