		...
		<table class="table table-striped table-bordered table-hover">
		    ...
		    <tbody v-if="dataService.length">
		        <tr v-for="(number, index) in dataService">
		            <td>
		            	<input type="text" :name="getName(index, 'icon')" v-model="number.icon" class="form-control">
		                <input type="hidden" :name="getName(index, 'id')" v-model="number.id">
		            </td>
		            <td>
		                <input type="text" :name="getName(index, 'heading')" v-model="number.heading" class="form-control">
		            </td>
		            <td>
		                <textarea :name="getName(index, 'description')" class="form-control" rows="3" v-model="number.description"></textarea>
		            </td>
		            <td>
		                <input type="text" 
		                	   class="form-control" 
