  ...
  Drupal.numericElement.attach = function(context, settings) {
    $('input.form-numeric:not(.form-numeric-processed)', context).addClass('form-numeric-processed').each(function() {
      ...
      $element.bind('keyup', function(e) {
        var kC = e.keyCode;
        if (kC > 47 && kC < 58 || kC > 95 && kC < 105 || kC == 8 || kC == 46) {
          // Vị trí trỏ chuột trong input
          var selection = e.target.selectionStart;
          var thsep = Drupal.settings.format_number.thousands_sep;
          var number = $element.val();
          // Đếm dấu phẩy của số ban đầu
          var cf_thsep = (number.match(new RegExp(thsep, 'g')) || []).length;
          if (thsep == '.') {
          ...
          number = number.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, thsep_rep);
          // Đếm dấu phẩy của số kết thúc
          var cl_thsep = (number.match(new RegExp(thsep, 'g')) || []).length;
          $element.val(number);
          this.setSelectionRange(selection + cl_thsep - cf_thsep, selection + cl_thsep - cf_thsep);
        }
      });
