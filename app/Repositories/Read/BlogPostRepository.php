...
class BlogPostRepository implements CRUDInterface {
    ...
    {
        $data['blog_cate'] = [];
        $blog_cate = BlogCategory::orderBy('sort_order', 'DESC')->get();
        if($blog_cate) {
            foreach($blog_cate as $blog_cate) {
                $blog_cate_description = $blog_cate->blog_category_descriptions()
    ...
    {
        $data['blog_post'] = [];
        $blog_posts         = BlogPost::orderBy('sort_order', 'DESC')->get();
        if($blog_posts) {
    ...
    public function edit( $id ) 
    {
		...
		foreach($blog_post->blog_post_descriptions as $blog_post_description) {
