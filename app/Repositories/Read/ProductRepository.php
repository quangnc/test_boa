...
class ProductRepository implements CRUDInterface {
    ...
    {
        $data['blog_post'] = [];
        $blog_posts         = Product::orderBy('sort_order', 'DESC')->get();
        if($blog_posts) {
            foreach($blog_posts as $blog_post) {
                $blog_post_description = $blog_post->product_descriptions()
                                                ->where('language_id', $this->adminLanguage())
                                                ->first();
                if($blog_post_description) {
                    $data['blog_post'][] = array(
                        'id'         => $blog_post->product_id,
    ...
    public function edit( $id ) 
    {
		...
		foreach($blog_post->product_descriptions as $blog_post_description) {
